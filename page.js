/* storing the json datas in jsondata constant variable */
const jsondata=[
    {
        "id": "1",
        "name": "Lura Senger",
    ...
    /* below piece of code is to include the jsondata into html tbody tags using DOM.
    Elements are appended into html using appendchild function */
    const tablebody=document.querySelector("#paginatedTable tbody");
    ...
    /* All the codes below is to implement pagenation from the above appended elements as rows in the table */
...
/* getnumberofpages calculate the total number of pages  */
function getNumberOfPages() {
  return Math.ceil(rows.length / numberPerPage);
}
/* generatePage number will create span element and append the span elements along with the pagenumber  */
...
/* this will jump to the respective pages by using loadrow and activepagenum function */
...
/* function will be called when next button is clicked */
function nextPage() {
  currentPage += 1;
  loadRows();
  activePageNum(currentPage);
}
/* function will be called when previou button is clicked */
function prevPage() {
  currentPage -= 1;
  loadRows();
  activePageNum(currentPage);
}
/* function will be called when first button is clicked */
function firstPage() {
  currentPage = 1;
  loadRows();
  activePageNum(currentPage);
}
/* function will be called when last button is clicked */
function lastPage() {
  currentPage = numberOfPages;
  loadRows();
  activePageNum(currentPage);
}
/* function will be called when number buttons are clicked */
function loadRows() {
